#!/bin/bash
# BARROS MAP CORE - Prova 1 V1.2.0 - build do mapa de Pernambuco
set -e
...
if [ "$JAR_SHA" != "$PLANETILER_SHA256_EXPECTED" ]; then
  echo "[REPROVADO] Checksum do Planetiler nao confere."; exit 1
fi
echo ">> Planetiler verificado. Gerando Pernambuco..."
mkdir -p public/tiles
OSM_RETRIEVED_AT=$(date -u +%Y-%m-%dT%H:%M:%SZ)
INICIO=$(date +%s)
java -Xmx4g -jar "$JAR" --area=pernambuco --download --output=public/tiles/barros-map-pe.pmtiles
FIM=$(date +%s)
...
echo ">> Mapa gerado: $((TAM/1048576)) MB - sha256 ${SHA}"
